...
const BOOKS = ['', 'In die Wildnis', 'Feuer und Eis', 'Geheimnisse des Waldes', 'Vor dem Sturm', 'Pfad der Gefahr', 'Stunde der Finsternis', 'Mitternacht', 'Mondschein', 'Morgenröte', 'Sternenglanz', 'Dämmerung', 'Sonnenuntergang', 'Der geheime Blick', 'Fluss der Finsternis', 'Verbannt', 'Zeit der Dunkelheit', 'Lange Schatten', 'Sonnenaufgang'];
const staffelLabel = ch => ch > 12 ? 'Staffel 3 · ' : ch > 6 ? 'Staffel 2 · ' : '';
...
const UI = {
  ...
  hud(dt) {
    ...
    else if (!Story.done()) { const q = Story.quest(); h += `<div class="ch">${staffelLabel(q.ch)}Buch ${q.ch} · ${q.title}</div><h4>➤ ${Story.text() || ''}</h4>`; }
...
const STAGE_ORDER = [['hauskaetzchen', 'Hauskätzchen'], ['schueler', 'Schüler'], ['krieger', 'Krieger'], ['zweiter', 'Zweiter Anführer'], ['anfuehrer', 'Anführer'], ['staffel2', 'Staffel 2'], ['staffel2b', 'Zweiter Anführer'], ['staffel3', 'Staffel 3'], ['staffel3b', 'Krieger Löwenglut'], ['generation', 'Neue Generation']];
function chronPanel() {
  const cards = STAGE_ORDER.filter(([k]) => G.stages[k] || !['generation', 'staffel2', 'staffel2b', 'staffel3', 'staffel3b'].includes(k)).map(([k, t]) => {
    const s = G.stages[k];
